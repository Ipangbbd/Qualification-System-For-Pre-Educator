...
## Project adaptation: EduQualify — Apple-like visual system

This repository adapts the Apple-inspired tokens above to a functional LMS onboarding flow. Key decisions taken in the codebase:

- Background: default canvas is `{colors.canvas}` / `{colors.canvas-parchment}` across most sections to keep the page bright and airy.
- Typography: hero headlines use `{typography.hero-display}` tightened and pushed to `font-weight: 700/800` for strong contrast; body copy remains at 17px for readable rhythm.
- Cards: utility and feature cards are borderless, use soft off-white backgrounds (`{colors.canvas-parchment}`), and adopt `{rounded.card}` (24px) to read as premium 'bento' tiles.
- Mockups: product/dashboard mockups use a glass-style surface with `backdrop-filter: blur(6px)` plus a very soft diffused shadow; the black solid mockup was replaced with translucent white to match the parchment canvas.
- Icons: feature icons are monochrome outline SVGs (no filled color badges) to reduce visual clutter and increase typographic emphasis.
- CTA: primary CTAs changed to dark (near-black) capsule buttons with white text for strong call-to-action parity across light tiles.
- Footer: reduced vertical padding and smaller fine-print (`{typography.fine-print}`) with a subtle divider (`{colors.surface-chip}`) to create a dense, scannable footer.

Implementation notes:

- See `src/index.css` for the new utilities: `.glass-mockup`, `.card-apple`, and softer `product-shadow`.
- See `src/components/HeroSection.jsx`, `FeaturesSection.jsx`, `CTASection.jsx`, `Footer.jsx` for component-level examples of the tokens applied.

TL;DR checklist applied in code:

- Background: `#ffffff` / `#f5f5f7`
- Cards: rounded 20–24px, borderless, soft fill
- Shadows: replaced heavy shadows with soft diffused shadows; product imagery retains the one stronger shadow rule
- Spacing: increased vertical padding for section breathing
